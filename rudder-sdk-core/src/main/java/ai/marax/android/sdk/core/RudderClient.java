...
public class RudderClient {
    ...
    public WebView drawOfferView(Context context) {
        ...
        offerWebView.addJavascriptInterface(new WebAppInterface(context), "Android");
...
//        properties.put("view_name", "Show all offers");
//        getRudderClient().track("VIEW_ALL_OFFERS", new RudderProperty().putValue(properties));
    }
    ...
    public class WebAppInterface {
        ...
        public void killView(String toast) {
            ...
            track("OFFER_ACCEPTED", new RudderProperty().putValue(properties));
        }
    }
    ...
    public void registerDevice(@NonNull String token, @NonNull String deviceId) {
        ...
        SharedPreferences pref = getApplication().getSharedPreferences(Utils.RUDDER_PREFS, Context.MODE_PRIVATE);
            ...
            public void run() {
                ...
                while (!isDone && retryCount <= 3) {
                    try {
                        ...
                        RudderLogger.logDebug(String.format(Locale.US, "RudderServerConfigManager: Hitting registration endpoint: configUrl: %s", configUrl));
                        ...
                        RudderLogger.logDebug(String.format(Locale.US, "RudderServerConfigManager: registerDevice: response status code: %d", httpConnection.getResponseCode()));
...
//                            String mId = convertedObject.get("success").getAsString();
                            ...
                            RudderLogger.logDebug(String.format(Locale.US, "RudderServerConfigManager: registerDevice: response maraxId : %s", mId));

                            // Add/update the maraxId in SharedPreferences
                            SharedPreferences preferences = getApplication().getSharedPreferences(Utils.RUDDER_PREFS, Context.MODE_PRIVATE);
                            SharedPreferences.Editor editor = preferences.edit();
                            editor.putString("maraxId", mId);
                            editor.apply();

                            isDone = true;
                            RudderLogger.logInfo("RudderServerConfigManager: Successfully updated the token");
                            RudderLogger.logDebug(String.format(Locale.US, "RudderServerConfigManager: registerDevice: updated maraxId: %s", preferences.getString("maraxId", "undefined")));
                            ...
                            // finally return response when reading from server is completed
                            RudderLogger.logError("EventRepository: registerDevice: ServerError: " + baos.toString());
            ...
            public void run() {
                ...
                while (!isDone && retryCount <= 3) {
                    try {
                        ...
                        RudderLogger.logDebug(String.format(Locale.US, "RudderServerConfigManager: Hitting update attribute endpoint: configUrl: %s", configUrl));
                        ...
                        // create connection
                        httpConnection.connect();
                        RudderLogger.logDebug(String.format(Locale.US, "RudderServerConfigManager: updateAttributes: response status code: %d", httpConnection.getResponseCode()));
                        if (httpConnection.getResponseCode() == 200) {
                            BufferedInputStream bis = new BufferedInputStream(httpConnection.getInputStream());
                            ByteArrayOutputStream baos = new ByteArrayOutputStream();
                            int res = bis.read();
                            ...
                            RudderLogger.logDebug(String.format(Locale.US, "RudderServerConfigManager: updateAttributes: response success : %s", success));

                            isDone = true;
                            RudderLogger.logInfo("RudderServerConfigManager: Successfully updated the attributes");
                            ...
                            // finally return response when reading from server is completed
                            RudderLogger.logError("EventRepository: updateAttributes: ServerError: " + baos.toString());
                            RudderLogger.logInfo("Retrying to download in " + retryTimeOut + "s");
                            retryCount += 1;
                            Thread.sleep(retryCount * retryTimeOut * 1000);
...
//            editor.putString("maraxId", "abcde");
//            editor.apply();
